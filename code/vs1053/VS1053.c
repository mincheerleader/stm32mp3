 ...
 void mp3_Init(void)
{
	//rst0;			             //硬件复位
	VS_RST_CLR;
	delay_vs(30000);			   // 大于2毫秒的延时,由于硬件复位的原因，所以必须要延时，详细参考数据手册
//	rst1;
	VS_RST_SET;
	delay_vs(30000);

	VS_SPI_SpeedLow();
//	command_write(0x00,0x08,0x97);			//软件复位,向0号寄存器写入0x0804   SM_SDINEW为1   SM_RESET为1
	VS_WR_Cmd(0x00,0x0804);
	delay_vs(30);
//	command_write(0x03,0x98,0x00);			//设置时钟，SC—MULT为4，SC—ADD为3，SC_FREQ为0
	VS_WR_Cmd(0x03,0x9800);
	delay_vs(30);
//	command_write(0x02,0x7a,0xf6);			//设置高低音
	VS_WR_Cmd(0x02,0x7af6);
	delay_vs(30);
//	command_write(0x0b,0x40,0x40);			//设置音量，(0x20为个人感觉)左右音量一样大,为32
